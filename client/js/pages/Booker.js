/**
 * Booker - Page class for booking/invoice management
...
export class Booker extends DataPage {
  ...
  /**
   * Get action buttons for booker page
   */
  getActionButtons() {
    return [
  ...
  populateBookingTable() {
    ...
    // AUTO-CALC: duration from startTime + endTime when duration is empty
    if (this.state.Booking.startTime && this.state.Booking.endTime && !this.state.Booking.duration) {
      const duration = DateTimeUtils.calculateDuration(this.state.Booking.startTime, this.state.Booking.endTime);
      if (duration !== null) {
        this.state.Booking.duration = duration;
      }
    }

    // AUTO-CALC: totalAmount from hourlyRate * duration (or flatRate)
    if (this.state.Booking.duration && !this.state.Booking.totalAmount) {
