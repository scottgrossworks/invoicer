...
export class Prisma_Sqlite_DB implements Leedz_DB {
  ...
  async createBooking(data: CreateBookingData): Promise<Booking> {
    // Check for duplicate: same client, same date, same address
    if (data.clientId && data.startDate && data.address) {
      const existing = await this.prisma.booking.findFirst({
        where: {
          clientId: data.clientId,
          startDate: data.startDate,
          location: data.address  // Note: schema uses 'location' field
        }
      });
      ...
      if (existing) {
        ...
        return await this.prisma.booking.update({
          ...
          data: {
            ...updateData,
            location: address,  // Map address to location field
    ...
    // Create new booking
    const { address, ...bookingData } = data;
